...
#                            pywraplp.Solver.CBC_MIXED_INTEGER_PROGRAMMING)
  ...
  # Capacity ctr
    ...
    for ij in range(num_links):
 #       capacityCtrs[ij] = solver.Constraint(0, capacity)
      capacityCtrs[ij] = solver.Constraint(-solver.infinity(), 0.0)
    ...
    for sd in range(num_flows):
        for ij in range(num_links):
            if x[sd, ij].solution_value() > 0:
                print('Flow %d assigned to link %d  Flow = %d  Reduced cost = %d  Basis status = %d' % (
                sd,
                ij,
                x[sd, ij].solution_value(),
                x[sd, ij].reduced_cost(),
                x[sd, ij].basis_status()))
    print()
    for ij in range(num_links):
